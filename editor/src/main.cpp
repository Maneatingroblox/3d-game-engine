...
void PrintUsage() {
    static const std::string usageText =
        std::string("Forgeworks Map Maker (build ") + FW_BUILD_ID + ")\n"
        "\n"
        ...
        "  --reset-layout           Delete the saved ImGui layout (imgui_layout.ini)\n"
        "                           and rebuild the default dock layout\n"
        ...
        "\n"
        "The window title shows the live renderer and the build, e.g.\n"
        "  Forgeworks Map Maker  [GPU rendering] a1b2c3d\n"
        "so a screenshot identifies the exact binary.";
    MessageBoxA(nullptr, usageText.c_str(), "Forgeworks Map Maker", MB_OK | MB_ICONINFORMATION);
}
